...
public class JsonUtil {
		...
		try {
			...
			for (String key : keys) {
				if (node.has(key)) {
					...
					throw new RuntimeException("key not exist in json str, keys:" + allKeys);
				}
			}
			return node.toString();
		} catch (JsonParseException e) {
			throw new RuntimeException("parse json error, json=" + str + ", keys:" + allKeys, e);
		} catch (IOException e) {
			throw new RuntimeException("parse json error, json=" + str + ", keys:" + allKeys, e);
		}
	}
	...
	public static String toPrettyString(Object o) {
		...
		if (o != null) {
			try {
				ret = objectMapper.defaultPrettyPrintingWriter().writeValueAsString(o);
	...
	public static <T> Map<String, T> convert(Map<String, Object> map, Class<T> clazz) {
		...
		if (map != null) {
			for (Map.Entry<String, Object> me : map.entrySet()) {
				...
				if (o != null) {
					String json = getJsonFromObject(o);
					T t = getObjectFromJson(json, clazz);
					ret.put(key, t);
				}
			}
		}
